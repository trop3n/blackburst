...
Stack: React 18 + TypeScript (strict) + Vite + Zustand + Tailwind v4, plus **Supabase (optional)** for auth, cloud persistence, and sharing. With no Supabase env set the app runs local-only (all state in the browser via `localStorage`); once configured it gates behind magic-link auth and persists per-user on the server. See the auth note below.

> **Auth & user accounts (implemented; dormant until Supabase is configured).** Magic-link auth, per-user cloud persistence, and project sharing now exist via Supabase. The master switch is `isSupabaseConfigured` (`src/lib/supabase.ts`, true when `VITE_SUPABASE_URL` + `VITE_SUPABASE_ANON_KEY` are set):
