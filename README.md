...
**AV / live-production design tooling that runs entirely in the browser.** Five dense, technical modules behind one keyboard-driven shell (rail · ⌘K command palette · status bar). No backend — every project lives in `localStorage` and exports to a portable JSON snapshot.
...
React 18 · TypeScript (strict) · Vite · Zustand · Tailwind v4.

No server and no database: each project's module state is persisted to `localStorage` and can be exported or imported as a `blackburst-project` JSON file. Switching projects snapshots the live stores and restores the target project's state.
